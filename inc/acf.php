 ...
 * @return bool
 */
function indy_display_content_blocks() {
	if ( have_rows( 'content_blocks' ) ) :
		while ( have_rows( 'content_blocks' ) ) :
			...
			if ( indy_has_block_expired( array(
 ...
 */
function indy_display_block_options( $args = array() ) {
 ...
 */
function indy_get_animation_class() {
 ...
 * @return bool
 */
function indy_has_block_expired( $args = array() ) {
 ...
 */
function indy_acf_admin_scripts() {
	...
	wp_enqueue_script( 'indy-admin-acf-scripts', get_template_directory_uri() . '/assets/scripts/admin-acf' . $suffix . '.js', array( 'jquery' ), $version, true );

	// Enqueue color picker styles.
	wp_enqueue_style( 'indy-admin-acf-styles', get_template_directory_uri() . '/admin-acf-styles.css', array(), $version );
}
add_action( 'acf/input/admin_head', 'indy_acf_admin_scripts' );
 ...
 */
function indy_acf_flexible_content_layout_title( $title, $field, $layout, $i ) {
	...
	$type = indy_return_flexible_content_layout_value( $background_type );
	...
	if ( 'video' === $background_type ) {
		$title .= '<div style="font-size: 30px; height: 26px; width: 30px;" class="dashicons dashicons-format-video acf-flexible-title-image"><span class="screen-reader-text">' . esc_html__( 'Video', 'indy' ) . '</span></div>';
	}
	...
	if ( indy_has_block_expired(
			array(
				'start_date' => $start_date,
				'end_date'   => $end_date,
			) )
	) {
		$expired .= '<span style="color: red;">&nbsp;(' . esc_html__( 'expired', 'indy' ) . ')</span>';
	}
...
add_filter( 'acf/fields/flexible_content/layout_title/name=content_blocks', 'indy_acf_flexible_content_layout_title', 10, 4 );
 ...
 */
function indy_return_flexible_content_layout_value( $type ) {
...
if ( function_exists( 'indy_acf_flexible_content_layout_title' ) ) {

	/**
	 * Set Admin Styles for Flexible Content Layout Image/Title in indy_acf_flexible_content_layout_title().
	 */
	function indy_flexible_content_layout_title_acf_admin_head() {
	?>
	<style type="text/css">
	...
	add_action( 'acf/input/admin_head', 'indy_flexible_content_layout_title_acf_admin_head' );
}
