 ...
 */
function indy_categorized_blog() {

	$category_count = get_transient( 'indy_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'indy_categories', $category_count );
	}
 ...
 */
function indy_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function indy_get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'            => '',
		'height'           => '',
	...
	if ( ! is_numeric( $width ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'indy' ) );
	}

	if ( ! is_numeric( $height ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'indy' ) );
	}

	if ( ! ctype_xdigit( $background_color ) ) {
		throw new Exception( esc_html__( 'Please provide a valid hex color value for background_color', 'indy' ) );
	}

	if ( ! ctype_xdigit( $text_color ) ) {
		throw new Exception( esc_html__( 'Please provide a valid hex color value for text_color', 'indy' ) );
	}
	...
	$alt = apply_filters( 'indy_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'indy' ) );
 ...
 * @return string
 */
function indy_get_placeholder_unsplash( $args = array() ) {
	$default_args = array(
		'width'    => '',
		'height'   => '',
		'category' => '',
 ...
 * @return string
 */
function indy_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'indy_header_scripts' );
 ...
 * @return string
 */
function indy_display_customizer_footer_scripts() {

	// Check for footer scripts.
	$scripts = get_theme_mod( 'indy_footer_scripts' );
